...
if (!empty($graphData['Store']) && !empty($stores)) 
{
    ...
    if($graphPageNumber == 0)
    {
        ...
        if (!empty($aData) && !empty($weeknumbers)) 
        {
            ...
            {
                $classGraph = 'col-lg-12';
                $text = '<style="font-size:12px;">Weekly Report for</style> <br/><style="font-size:14px;">Merchant</style><br/><style="font-size:12px;">Week ' . $fromWeekNumber . ' - Week ' . $endFromWeekNumber . '</style>';
            }
            else 
            {
