...
if (!empty($graphData['Store']) && !empty($stores)) {
    ...
    if($graphPageNumber == 0)
    {
        ...
        if (!empty($aData) && !empty($oData) && !empty($datee)) {//Total result
            ...
            $text = '<style="font-size:12px;">Monthly Report For</style> <br/><style="font-size:14px;">HQ</style><br/><style="font-size:12px;">' . str_replace('\'', '', $datee[$month]) . ' - ' . $year . ' to ' . str_replace('\'', '', $datee[$toMonth]) . ' - ' . $toYear;
    ...
    foreach($pageMerchant[$graphPageNumber] as $keyStore => $valueStore)
    {
        ...
        $tamnt = implode(',', $tamntt);
        $torders = implode(',', $torders);
        $text = '<style="font-size:12px;">Monthly Report For</style> <br/><style="font-size:14px;">' . addslashes($valueStore) . '</style><br/><style="font-size:12px;">' . str_replace('\'', '', $datee[$month]) . ' - ' . $year . ' to ' . str_replace('\'', '', $datee[$toMonth]) . ' - ' . $toYear;
        ?>
        <div class="col-lg-4">
