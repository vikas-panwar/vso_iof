...
if (!empty($graphData['Store']) && !empty($stores)) 
{
    ...
    if($graphPageNumber == 0)
    {
        ...
        if (!empty($aData) && !empty($weeknumbers)) 
        {
            ...
            $subTitle = '<style="font-size:14px;font-weight:bold;">Total ' . $totalOffer . ' Extended Offers </style>';
            ?>
            <div class="col-lg-4">
                <div id="cTotal"></div>
            </div>
...
<div id="pagination_data_request">
    <?php echo $this->element('superNewReports/extended_promo/paginationall'); ?>
</div>
