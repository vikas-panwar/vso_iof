    ...
    <thead>
        <tr>
            <th class="th_checkbox" colspan="5" style="text-align:left;">
                Order Id : <?php echo $orderDetail['Order']['order_number']; ?>
                | Cost : <?php echo $this->Common->amount_format($total_amount);?>
                <?php if ($orderDetail['Order']['tax_price']) {
                    echo "| Tax :" . $this->Common->amount_format($orderDetail['Order']['tax_price']);
                } ?>
                | Status : <?php echo $data = $this->requestAction('/hqorders/ajaxRequest/' . $orderDetail['Order']['order_status_id'] . '') ?>
...
<?php echo $orderAttr['address']; ?>
                ...
                <?php if (!empty($orderDetail['OrderPayment']['last_digit'])) { ?>
                    &nbsp;| Card Number : <?php echo 'XXXXXXXXXXXX'.$orderDetail['OrderPayment']['last_digit']; ?>
                <?php } ?>
...
<td class="net-record-font-size"><?php echo number_format($orderDetail['Order']['tax_price'], 2); ?></td>-->
        ...
        <tr class="table-net-tr">
            ...
            <td class="net-record-font-size" style="width: 15%"><?php echo (($orderDetail['Order']['tax_price']) ? $this->Common->amount_format($orderDetail['Order']['tax_price']) : "-"); ?></td>
        </tr>

        <?php
        if (!empty($orderDetail['OrderItemFree'])) {
            foreach ($orderDetail['OrderItemFree'] as $fkey => $itemfree) {
                ?>     
                <tr>
        ...
        <tr class="table-net-tr">
            ...
            <td class="table-net-td">&nbsp;</td>
            <td class="table-net-td">&nbsp;</td>
            <td class="net-record-font-size">Coupon Discount:<br><?php echo ($orderDetail['Order']['coupon_code']) ? "(" . $orderDetail['Order']['coupon_code'] . ")" : ''; ?></td>
            <td class="net-record-font-size"
                style="color:#ff1a1a;"><?php echo ($orderDetail['Order']['coupon_discount']) ? "-" . $this->Common->amount_format($orderDetail['Order']['coupon_discount']) : '-'; ?></td>
        </tr>
        ...
        <tr class="table-net-tr">
            ...
            <td class="net-record-font-size"><?php echo ($orderDetail['Order']['service_amount']) ? $this->Common->amount_format($orderDetail['Order']['service_amount']) : '-'; ?></td>
        </tr>
<?php if ($orderDetail['Segment']['id'] != 2) { ?>
            <tr class="table-net-tr">
                ...
                <td class="net-record-font-size"><?php echo ($orderDetail['Order']['delivery_amount']) ? $this->Common->amount_format($orderDetail['Order']['delivery_amount']) : '-'; ?></td>
            </tr>
<?php } ?>
        ...
        $tipLabel = '';
        $tipAmount = '';
        if($orderDetail['Order']['tip'] > 0 && $orderDetail['Order']['tip_option'] == 0 && $orderDetail['Order']['tip_percent'] == 0)
        {
            $tipLabel = 'Tip: ';
            $tipAmount = $this->Common->amount_format($orderDetail['Order']['tip']);
        } else {
            if($orderDetail['Order']['tip_option'] == 0) {
                $tipLabel = 'No Tip';
                $tipAmount = '';
            } else if($orderDetail['Order']['tip_option'] == 1) {
                $tipLabel = 'Tip With Cash';
                $tipAmount = '';
            } else if($orderDetail['Order']['tip_option'] == 2) {
                $tipLabel = 'Tip With Card: ';
                $tipAmount = $this->Common->amount_format($orderDetail['Order']['tip']);
            } else {
                $tipLabel = 'Tip % (' . $orderDetail['Order']['tip_percent'] . '%): ';
                $tipAmount = $this->Common->amount_format($orderDetail['Order']['tip']);
            }
        }
        ?>
