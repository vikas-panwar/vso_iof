<?php
/**
 * Application level View Helper
 *
 * This file is application-wide helper file. You can put all
 * application-wide helper-related methods here.
 *
 * CakePHP(tm) : Rapid Development Framework (http://cakephp.org)
 * Copyright (c) Cake Software Foundation, Inc. (http://cakefoundation.org)
 *
 * Licensed under The MIT License
 * For full copyright and license information, please see the LICENSE.txt
 * Redistributions of files must retain the above copyright notice.
 *
 * @copyright     Copyright (c) Cake Software Foundation, Inc. (http://cakefoundation.org)
 * @link          http://cakephp.org CakePHP(tm) Project
 * @package       app.View.Helper
 * @since         CakePHP(tm) v 0.2.9
 * @license       http://www.opensource.org/licenses/mit-license.php MIT License
 */

App::uses('Helper', 'View');

/**
 * Application helper
 *
 * Add your application-wide methods in the class below, your helpers
 * will inherit them.
 *
 * @package       app.View.Helper
 */ 
 
 
 
class DateformHelper extends Helper {
    
        /*------------------------------------------------
        Function name:us_format()
        Description:To format date in m-d-y format
        created:22/7/2015
       -----------------------------------------------------*/
    
        public function us_format($userDate=null){
            if($userDate){
                
                
                 if($userDate){
                    $userDate=str_replace('-','/',$userDate);  
                    $correctDate=date('m-d-Y',strtotime($userDate));
                
                    if($correctDate=="1970-01-01"){
                       $dateArray=explode("-",$userDate);
                       $correctDate=$dateArray[2]."-".$dateArray[0]."-".$dateArray[1];
                       
                    }
                
                   
                }
                 return $correctDate;
            }
        }
        
        
        public function us_formatwithTime($userDate=null){
            if($userDate){
                
                
                 if($userDate){
                    $userDate=str_replace('-','/',$userDate);
                    //$correctDate=DATE_FORMAT($userDate,'%m-%d-%Y %T');
                    $correctDate=date('m-d-Y H:i:s A',strtotime($userDate));
                
                    if($correctDate=="1970-01-01"){
                       $dateArray=explode("-",$userDate);
                       $correctDate=$dateArray[2]."-".$dateArray[0]."-".$dateArray[1];
                       
                    }
                
                   
                }
                 return $correctDate;
            }
        }
}