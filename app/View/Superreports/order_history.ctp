  ...
  <div class="tab-content">
   ...
   <!--******************Order details area start here***********************-->
    ...
    <div role="tabpanel" class="tab-pane" id="order">
	 ...
	 ?></h3><h4>Total Amount : <span id="assign"><?php  echo number_format($total_amount,2); ?></span></h4>	 
	 <br>
	...
	$status_name =  $this->requestAction('/customers/ajaxRequest/'.$data['Order']['order_status_id'].'');
		...
		<br>
		Address : <?php  if($data['Order']['seqment_id']==2){echo "Pickup";}else{ echo $data['DeliveryAddress']['city']." ".$data['DeliveryAddress']['address']; }?>
		
		
		<br>
	       Order Type : <?php  echo $data['Segment']['name'] ;?>  |  Created : &nbsp;&nbsp;<?php  echo $data['Order']['created'] ; ?> |  Delivery Time : &nbsp;&nbsp;<?php  echo $data['Order']['pickup_time'] ;?>
					</th>
				</tr>
			</thead>
		</table>
...
$('#myTabs li:eq(2) a').tab('show') // Select third tab (0-indexed)</script><style>
.rating-xs {
    float: left;
    font-size: 1.5em;
    margin-left: 15px;
