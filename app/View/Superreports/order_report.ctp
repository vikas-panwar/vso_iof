...
$amntdate = ''; $amnt = '' ; $tamnt = 0 ;
    if($type == 1){       
       echo $this->element('/superreport/order/daily');
    }elseif($type == 2){
        //echo "In progress";
       echo $this->element('/superreport/order/weekly'); 
    }elseif($type == 3){
       echo $this->element('/superreport/order/monthly'); 
    }elseif($type == 4){
       echo $this->element('/superreport/order/yearly'); 
    }elseif($type == 5){
       echo $this->element('/superreport/order/life_time'); 
    } 
...
<div class="row">
    <div class="col-lg-12">
        ...
        <?php echo $this->Form->create('Report', array('url' => array('controller' => 'superreports', 'action' => 'orderReport','id'=>'one')));  ?>
	...
	<?php echo $this->Form->input('Segment.id',array('type'=>'select','class'=>'form-control valid','label'=>false,'div'=>false,'autocomplete' => 'off','options'=>$typeList,'empty'=>'Order Type')); ?>		
                ...
                <div class="col-lg-1">	
                    ...
                    <?php
		      if(!empty($storeId)){
		    echo $this->Html->link('Download Excel', array('controller'=>'superreports','action'=>'orderReportDownload',$storeId,$type,$startdate,$enddate,$Month,$Year,$yearFrom,$yearTo,$ordertype),array('class' => 'btn btn-default'));
		    
		      }else{
			
		    echo $this->Html->link('Download Excel', array('controller'=>'superreports','action'=>'superorderReportDownload',$type,$startdate,$enddate,$Month,$Year,$yearFrom,$yearTo,$ordertype),array('class' => 'btn btn-default'));
	  ...
	  echo  $this->element('superreport/order/index') ;
