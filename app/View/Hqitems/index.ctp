<div class="row">
    <div class="col-lg-12">
        ...
        <div class="table-responsive">
            <?php echo $this->Form->create('Item', array('url' => array('controller' => 'hqitems', 'action' => 'index'), 'id' => 'AdminId', 'type' => 'post')); ?>
            <div class="row padding_btm_20">
                <div class="col-lg-3">
                    <?php
                    $merchantList = $this->Common->getHQStores($this->Session->read('merchantId'));
                    echo $this->Form->input('Item.store_id', array('options' => @$merchantList, 'class' => 'form-control', 'label' => false, 'div' => false, 'empty' => 'All Store'));
                    ?>
                </div>
            ...
            <div class="row padding_btm_20">
                ...
                <div class="col-lg-4">
                    <div class="addbutton">
                        <?php echo $this->Form->button('Add Menu Item', array('type' => 'button', 'onclick' => "window.location.href='/hqitems/addMenuItem'", 'class' => 'btn btn-default')); ?>
                        ...
                        <?php //echo $this->Form->button('Upload Menu', array('type' => 'button', 'onclick' => "window.location.href='/hqitems/uploadfile'", 'class' => 'btn btn-default', 'style' => 'margin-top:5px;')); ?>
                        <?php echo $this->Html->link('Clear', array('controller' => 'hqitems', 'action' => 'index', 'clear'), array('class' => 'btn btn-default')); ?>
                    </div>
                </div>
            </div>
            <?php echo $this->Form->end(); ?>
            <?php echo $this->element('show_pagination_count'); ?>
            <?php echo $this->Form->create('Item', array('url' => array('controller' => 'hqitems', 'action' => 'deleteMultipleItem'), 'id' => 'OrderId', 'type' => 'post')); ?>
                    ...
                    if ($list) {
                        ...
                        foreach ($list as $key => $data) {
                            ...
                            <tr <?php echo $class; ?> notif-id="<?php echo $EncryptItemID; ?>">
                                ...
                                <td><?php
                                    if (!empty($data['Item']['store_id'])) {
                                        echo $this->Dateform->us_format($this->Hq->storeTimezone(null, $data['Item']['created'], null, $data['Item']['store_id']));
                                    } else {
                                        echo $this->Dateform->us_format($data['Item']['created']);
                                    }
                                    //echo $this->Dateform->us_format($data['Item']['created']);
                                    ?></td>
                                <td>
                                    ...
                                    if ($data['Item']['is_active']) {
                                        echo $this->Html->link($this->Html->image("store_admin/active.png", array("alt" => "Active", "title" => "Active")), array('controller' => 'hqitems', 'action' => 'activateItem', $EncryptItemID, 0), array('confirm' => 'Are you sure to Deactivate Item?', 'escape' => false));
                                    } else {
                                        echo $this->Html->link($this->Html->image("store_admin/inactive.png", array("alt" => "Inactive", "title" => "Inactive")), array('controller' => 'hqitems', 'action' => 'activateItem', $EncryptItemID, 1), array('confirm' => 'Are you sure to Activate Item?', 'escape' => false));
                                    }
                                    ?>
                                </td>
                                <td style="width:150px;" class='sort_order'>
                                    <?php //$EncryptStoreID=$this->Encryption->encode($data['User']['id']);  ?>
                                    <?php echo $this->Html->link($this->Html->image("store_admin/edit.png", array("alt" => "Edit", "title" => "Edit")), array('controller' => 'hqitems', 'action' => 'editMenuItem', $EncryptItemID), array('escape' => false)); ?>
                                    <?php echo " | "; ?>
                                    <?php echo $this->Html->link($this->Html->image("store_admin/delete.png", array("alt" => "Delete", "title" => "Delete")), array('controller' => 'hqitems', 'action' => 'deleteItem', $EncryptItemID), array('confirm' => 'Are you sure to delete Item?', 'escape' => false)); ?>
                                    <?php echo " | "; ?>
                                    <?php
                                    if (isset($data['ItemPrice']) && !empty($data['ItemPrice'][0]['size_id']))
                                        echo $this->Html->link('Price', array('controller' => 'hqitems', 'action' => 'setPrice', $EncryptItemID));
                                    ?>
                                    <?php
...
<script>
    $(document).ready(function () {
        ...
        $("#ItemKeyword").autocomplete({
            source: "/hqitems/getSearchValues?storeID=" + storeId,
            minLength: 3,
        ...
        $("#ItemStoreId,#ItemStoreId").change(function () {
...
<script>
    ...
    function updateOrder() {
        ...
        if (orderData) {
            $.ajax({
                url: '/hqitems/updateOrder?' + orderData,
                type: 'get',
                success: function () {
                }
            });
